;; Security Audit Rating System

(define-constant ERR-NOT-AUTHORIZED (err u800))
(define-constant ERR-AUDIT-NOT-FOUND (err u801))
(define-constant ERR-ALREADY-VOTED (err u802))
(define-constant ERR-INVALID-RATING (err u803))
(define-constant ERR-INSUFFICIENT-REPUTATION (err u804))

(define-constant AUDIT-PENDING u1)
(define-constant AUDIT-COMPLETED u2)
(define-constant AUDIT-DISPUTED u3)

(define-constant MIN-AUDITOR-REPUTATION u500)
...
(define-public (submit-audit 
  ...
  (let ((audit-id (var-get audit-id-counter))
    ...
    (begin
      ...
      (asserts! (>= (get reputation (unwrap-panic auditor-data)) MIN-AUDITOR-REPUTATION) ERR-INSUFFICIENT-REPUTATION)
      (asserts! (and (>= initial-rating u1) (<= initial-rating u10)) ERR-INVALID-RATING)
      (asserts! (and (>= severity-score u0) (<= severity-score u100)) ERR-INVALID-RATING)
...
(define-public (vote-on-audit (audit-id uint) (rating-vote uint) (confidence-level uint))
  (let ((audit (unwrap! (map-get? security-audits { audit-id: audit-id }) ERR-AUDIT-NOT-FOUND))
    ...
    (begin
      ...
      (asserts! (>= (get reputation auditor-data) MIN-AUDITOR-REPUTATION) ERR-INSUFFICIENT-REPUTATION)
      (asserts! (and (>= rating-vote u1) (<= rating-vote u10)) ERR-INVALID-RATING)
      (asserts! (and (>= confidence-level u1) (<= confidence-level u100)) ERR-INVALID-RATING)
      (asserts! (is-eq (get status audit) AUDIT-PENDING) ERR-NOT-AUTHORIZED)
...
(define-public (dispute-audit (audit-id uint) (dispute-reason (string-ascii 100)))
  (let ((audit (unwrap! (map-get? security-audits { audit-id: audit-id }) ERR-AUDIT-NOT-FOUND))
        (auditor-data (unwrap! (map-get? certified-auditors tx-sender) ERR-NOT-AUTHORIZED)))
    (begin
      (asserts! (>= (get reputation auditor-data) (* MIN-AUDITOR-REPUTATION u2)) ERR-INSUFFICIENT-REPUTATION)
      (asserts! (is-eq (get status audit) AUDIT-COMPLETED) ERR-NOT-AUTHORIZED)
      
      (map-set security-audits
        { audit-id: audit-id }
        (merge audit { status: AUDIT-DISPUTED }))
      (ok true))))
...
(define-read-only (is-certified-auditor (auditor principal))
  (let ((auditor-data (map-get? certified-auditors auditor)))
    (if (is-some auditor-data)
      (>= (get reputation (unwrap-panic auditor-data)) MIN-AUDITOR-REPUTATION)
      false)))
